...
public class robot : Agent
{
    ...
    private void Start()
    {
        rigRobot = GameObject.Find("球拍").GetComponent<Rigidbody>();
        rigball = GameObject.Find("球").GetComponent<Rigidbody>();
    }
    ...
    public override void OnEpisodeBegin()
    {
        ...
        // 隨機機器人位置
        Vector3 posRobot = new Vector3(Random.Range(1f, 1f), 0.1f, Random.Range(-2f, -2f));
        transform.position = posRobot;

        // 隨機足球位置
        Vector3 posball = new Vector3(Random.Range(-1f, 1f), 0.1f, Random.Range(1f, 1f));
    ...
    public override void OnActionReceived(float[] vectorAction)
    {
        ...
        if (transform.position.y < -5 || rigball.position.y < -5)
